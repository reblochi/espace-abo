...
import { DashboardStats, NewsTips, ServicesPublics, CarburantsPrix, LaPoste } from '@/components/dashboard';
...
export default function DashboardPage() {
  ...
  const stats = [
    ...
    {
      label: isActive && subscription?.startDate
        ? `Membre FG depuis le ${new Date(subscription.startDate).toLocaleDateString('fr-FR')}`
        : 'Non abonné',
      value: isActive ? '✓' : '—',
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
