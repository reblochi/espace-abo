// Formulaire complet pour acte de mariage
...
import {
  ...
  type MarriageCertificateInput,
} from '@/schemas/marriage-certificate';
...
const PARENTS_STEP: Step = {
  id: 'parents',
  title: 'Parents',
  description: 'Informations des parents de l\'epoux(se) 1',
};

const SPOUSE_PARENTS_STEP: Step = {
  id: 'spouseParents',
  title: 'Parents 2',
  description: 'Informations des parents de l\'epoux(se) 2',
};
...
export function MarriageCertificateForm({
  ...
  const methods = useForm<MarriageCertificateInput>({
    resolver: zodResolver(marriageCertificateSchema),
    defaultValues: {
      recordType: RecordType.COPIE_INTEGRALE,
      recordCount: 3,
      marriageDate: '',
      marriageCountryId: FRANCE_COUNTRY_ID,
      marriageCityId: undefined,
      marriageCityName: '',
      gender: undefined,
      firstName: '',
      lastName: '',
  ...
  const showParentSteps = recordType === RecordType.COPIE_INTEGRALE
    || recordType === RecordType.EXTRAIT_FILIATION;
    ...
    const steps: Step[] = [
      {
        id: 'eventInfo',
        title: 'Mariage',
        description: 'Type d\'acte et informations sur le mariage',
      },
      {
        id: 'beneficiary',
        title: 'Epoux(se) 1',
        description: 'Informations de la premiere personne concernee',
      },
    ];

    if (showParentSteps) {
      steps.push(PARENTS_STEP);
    }

    steps.push({
      id: 'spouse',
      title: 'Epoux(se) 2',
      description: 'Informations de la seconde personne concernee',
    });

    if (showParentSteps) {
      steps.push(SPOUSE_PARENTS_STEP);
    }
  ...
  const tracking = useFormTracking({
    formType: 'CIVIL_STATUS_MARRIAGE',
  ...
  const stepFieldsMap: Record<string, (keyof MarriageCertificateInput)[]> = {
    eventInfo: ['recordType', 'recordCount', 'marriageDate', 'marriageCountryId', 'marriageCityName'],
  ...
  const handleFormSubmit = async (data: MarriageCertificateInput) => {
    ...
    try {
      if (isEmbed) {
        const response = await fetch('/api/embed/acte-mariage', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
