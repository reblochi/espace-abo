...
export function StepSummary({
  ...
  const { watch, setValue, formState: { errors } } = useFormContext<MarriageCertificateInput>();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <SummaryCard title="Date et lieu du mariage">
          <p className="text-base font-medium">{formatDate(formData.marriageDate)}</p>
          <p className="form-gov-hint">
            {formData.marriageCityName}
            {formData.marriageCountryId === FRANCE_COUNTRY_ID ? '' : ' (etranger)'}
          </p>
        </SummaryCard>

        <SummaryCard title="Epoux(se) 1">
          <p className="text-base font-medium">
            {formData.gender && genderLabels[formData.gender]} {formData.firstName} {formData.lastName}
          </p>
          {formData.nomUsage && (
            <p className="form-gov-hint">Usage : {formData.nomUsage}</p>
          )}
        ...
        <SummaryCard title="Epoux(se) 2">
          <p className="text-base font-medium">
            {formData.spouseGender && genderLabels[formData.spouseGender]} {formData.spouseFirstName} {formData.spouseLastName}
          </p>
          {formData.spouseNomUsage && (
            <p className="form-gov-hint">Usage : {formData.spouseNomUsage}</p>
          )}
          <p className="form-gov-hint">Ne(e) le {formatDate(formData.spouseBirthDate)}</p>
        </SummaryCard>

        {formData.claimerType && (
          <SummaryCard title="Lien avec les epoux">
            <p className="text-base">{claimerTypeLabels[formData.claimerType]}</p>
          </SummaryCard>
        )}

        {showParents && (
          <SummaryCard title="Parents de l'epoux(se) 1">
        ...
        {showParents && (
          <SummaryCard title="Parents de l'epoux(se) 2">
            <p className="text-base">
              Pere : {formData.spouseFatherUnknown ? <span className="italic text-gray-400">Inconnu</span> : `${formData.spouseFatherFirstName} ${formData.spouseFatherLastName}`}
            </p>
            <p className="text-base">
              Mere : {formData.spouseMotherUnknown ? <span className="italic text-gray-400">Inconnue</span> : `${formData.spouseMotherFirstName} ${formData.spouseMotherLastName}`}
            </p>
          </SummaryCard>
        )}
